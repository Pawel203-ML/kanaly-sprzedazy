...
class db:
    ...
    def transation_init():
        ...
        transactions['tran_date'] = transactions['tran_date'].apply(lambda x: 
                                                convert_dates(x))
...
def tab1_bar_sales(start_date,end_date):
    ...
    grouped = truncated[truncated['total_amt']>0].groupby(
        [pd.Grouper(key='tran_date',freq='M'),'Store_type'])['total_amt'].sum().round(2).unstack()
    ...
    data = traces
    fig = go.Figure(data=data,layout=go.Layout(title='Przychody',barmode='stack',
...
def tab1_choropleth_sales(start_date,end_date):
    ...
    data = [trace0]
    fig = go.Figure(data=data,layout=go.Layout(title='Mapa',geo=dict(showframe=False,
...
def tab2_barh_prod_subcat(chosen_cat):
    ...
    grouped = df.merged[(df.merged['total_amt']>0)&(df.merged['prod_cat']==chosen_cat)].pivot_table(index='prod_subcat'
